...
export function EventsPanel() {
  const { plan, addEvent, updateEvent, deleteEvent, reorderEvents, deletePresetGroup, toggleEventHidden } = usePlanStore();
                  ...
                  <tbody>
                    {displayedEvents.map((event) => (
                      <SortableEventRow
                        ...
                        onToggleHidden={() => toggleEventHidden(event.id)}
                        onDeletePreset={
      ...
      <td className="px-3 py-2">
        <div className="flex gap-2 flex-wrap items-center">
          <button
            ...
            aria-label={event.hidden ? "Zobrazit v simulaci" : "Skrýt ze simulace"}

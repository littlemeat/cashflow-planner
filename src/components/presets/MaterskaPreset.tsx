...
export function MaterskaPreset({ onClose }: MaterskaPresetProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    batchUpdate((plan) => {
      // Add the PPM maternity benefit event
      ...
      // Optionally end an existing income event at the month before maternity starts
      if (endEventId !== "none" && wizardStartOffset > 0) {
        events = events.map((e) =>
          e.id === endEventId ? { ...e, endMonth: wizardStartOffset - 1 } : e
        );
      }
